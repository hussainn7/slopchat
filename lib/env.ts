...
export function getBaseUrl(): string {
  // PUBLIC_URL is the externally-reachable URL used for:
  //   - Instagram OAuth redirect URI (must match what's registered in Meta)
  //   - Meta webhook callback URL
  //   - Tracked link redirects in DMs
  // NEXTAUTH_URL is kept as http://localhost:3000 so email magic links always
  // land on localhost — no matter how often ngrok restarts and changes URL.
  return (
...
export const serverEnvSchema = z.object({
  ...
  // PUBLIC_URL is optional — falls back to NEXTAUTH_URL when not set.
